...
export default function AdminPaiements() {
  ...
  const fetchCommandes = async (pageNumber = 1) => {
    try {
      const res = await fetch(`${API_URL}/api/admin/commandes?page=${pageNumber}&limit=10`);
  ...
  const confirmerPaiement = async (commandeId, paiementRecuId) => {
    try {
      const res = await fetch(`${API_URL}/api/admin/commandes/${commandeId}/valider-paiement`, {
        method: "PUT",
              ...
              <td>
                ...
                {commande.paiementsRecus.map((p) => (
                  <div key={p._id} style={{ marginBottom: "0.5rem" }}>
                    ...
                    <br />
                    {p.status === "PENDING" && (
                      <Button className="confirm" onClick={() => confirmerPaiement(commande._id, p._id)}>Confirmer</Button>
                    )}
                  </div>
                ))}
              </td>
