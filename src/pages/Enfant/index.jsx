...
const ModalTitle = styled.h2` font-size: 1.5rem; margin-bottom: 1rem; `;
const ModalText = styled.p` margin-bottom: 1.5rem; `;
...
const FiltersWrapper = styled.div` display: flex; flex-wrap: wrap; gap: 1rem; `;
...
const Badge = styled.div`
  position: absolute; top: 12px; left: 12px; padding: 4px 10px; border-radius: 6px; font-size: 0.75rem; font-weight: 700; color: white;
...
const ViewButton = styled(Link)`
  position: absolute; bottom: 12px; left: 50%; transform: translateX(-50%); padding: 6px 12px; border-radius: 8px;
  text-decoration: none; font-weight: 600; font-size: 0.85rem; background: ${({ theme }) => theme.primary || "#007bff"};
  color: white; opacity: ${({ visible }) => (visible ? 1 : 0)}; pointer-events: ${({ visible }) => (visible ? "auto" : "none")}; transition: all 0.3s ease;
`;
const FavoriteButton = styled.button` border: none; background: transparent; cursor: pointer; font-size: 1.2rem; color: ${({ favorite }) => (favorite ? "#ef4444" : "#000")}; `;
...
function Enfant() {
  ...
  const filteredProducts = useMemo(() => {
    let data = products;
    if (filter !== "tous") data = data.filter(p => p.categorie === filter || p.badge === filter);
    return data;
  }, [filter, products]);
    ...
    <>
      <ModalOverlay visible={showModal}>
        <ModalContent>
          ...
          <div>
            <ModalButton primary onClick={() => navigate("/connexion")}>Se connecter / S'inscrire</ModalButton>
      ...
      <PageWrapper onClick={() => setActiveView(null)}>
        ...
        <FiltersWrapper>
          {filters.map(f => (
            <FilterButton key={f.value} active={filter === f.value} onClick={() => setFilter(f.value)}>{f.label}</FilterButton>
          ))}
        </FiltersWrapper>
        ...
        <Grid>
          {filteredProducts.map(p => {
            ...
            return (
              <ProductCard key={p._id} onClick={(e) => { e.stopPropagation(); setActiveView(p._id); }}>
                <ProductImageWrapper>
                  <ProductImage src={p.imageUrl[0]} alt={p.title} />
                  {p.badge && <Badge type={p.badge}>{p.badge}</Badge>}
                  <ViewButton to={`/produit/${p._id}`} visible={activeView === p._id}>Voir produit</ViewButton>
                </ProductImageWrapper>
