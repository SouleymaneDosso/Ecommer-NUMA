...
const NewsletterSection = styled.div`
  ...
  text-align: center;
`;
const NewsletterTitle = styled.h3`font-size: 1.4rem; font-weight: 700;`;
...
const FooterLink = styled(Link)`
  text-decoration: none; color: inherit; font-weight: 500; display: flex; align-items: center; gap: 6px;
  transition: color 0.25s ease, transform 0.25s ease, box-shadow 0.3s ease;
  &:hover { color: ${({ $isdark }) => ($isdark ? "#a5b4fc" : "#4f46e5")}; transform: translateX(4px); animation: ${glow} 0.8s ease; }
`;
...
const CookieButton = styled.button`
  background: none;
  border: 1px solid ${({ $isdark }) => ($isdark ? "#4f46e5" : "#4f46e5")};
  color: ${({ $isdark }) => ($isdark ? "#4f46e5" : "#4f46e5")};
  border-radius: 8px;
  padding: 6px 12px;
  cursor: pointer;
  width: fit-content;
...
`;
const ModalTitle = styled.h2`margin-bottom: 1rem; font-size: 1.5rem;`;
